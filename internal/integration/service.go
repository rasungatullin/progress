...
func resolvePrivateSystemConfig(ctx context.Context, system string, config *model.IntegrationSystemConfig, store secrets.Reader) error {
	...
	if strings.TrimSpace(config.TokenPrivate) != "" {
		...
		if err != nil {
			...
			return secrets.MaskError(fmt.Errorf("integration system %q cannot read private value %q", normalizeSystem(system), strings.TrimSpace(config.TokenPrivate)), value)
		}
...
func resolvePrivateGitHubAppConfig(ctx context.Context, system string, config *model.IntegrationSystemConfig, store secrets.Reader) error {
	...
	if err != nil {
		...
		return secrets.MaskError(fmt.Errorf("integration system %q cannot read private value %q", normalizeSystem(system), strings.TrimSpace(config.GitHubAppPrivateKeyPrivate)), value)
	}
	if strings.TrimSpace(value) == "" {
