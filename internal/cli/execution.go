...
func bindStructuredInputFlags(cmd *cobra.Command, flags *actionFlags) {
	...
	cmd.Flags().StringVar(&flags.baseRef, "base", "", "Базовая ветка запроса на слияние и нового рабочего места")
	cmd.Flags().StringVar(&flags.headRef, "head", "", "Ветка с изменениями для запроса на слияние; для start-implementation-pr должна совпадать с номером задачи")
