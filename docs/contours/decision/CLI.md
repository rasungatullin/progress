...
progress decision start --task 123 --route task-description
```

Если `--route` не задан, контур принятия решения использует ссылку `default_route` из каталога методик. Пустое значение не преобразуется в строковый маршрут `default`.
...
- приоритет;
- причины выбора;
- выбранное действие, если выбран режим `execute`;
...
- задание на выполнение для выбранного действия;
- минимальный набор ограничений для последующей стадии;
- пригодный для передачи в `progress execution action` блок данных.
...
- `--route` — имя маршрута обработки для явного выбора участка работы;
...
- причины выбора или отклонения конкретного действия;
...
3. при `execute` возвращается выбранное действие.
...
3. результат пригоден для последующей передачи в `progress execution action`.
...
- `decision plan` должен формировать структуру, пригодную для передачи в `progress execution action`;
