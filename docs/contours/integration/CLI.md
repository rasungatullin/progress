...
1. поле `token` больше не поддерживается: при чтении переходной конфигурации непустое значение отклоняется с сообщением о переходе на хранилище приватных значений и ссылку `token_private`;
