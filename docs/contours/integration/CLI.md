...
Если потребуется единообразие полей, адаптер должен сам преобразовать ответ в массив `TrackerComment`.
...
1. если операция требует репозиторий, `--repo` обязателен, кроме `github repo get` и `github issue get`, где можно опустить `--repo` и использовать `default_repo` из конфигурации;
...
5. `default_repo` позволяет упростить ручные вызовы CLI для `github repo get` и `github issue get`, если `--repo` не передан.
