...
Отключённая система может присутствовать в каталоге только с `available=false`. Сценарные системы типа `script` публикуют операции из `systems.<name>.operations`; если система включена, такие операции считаются доступными через сценарный адаптер и сохраняют контракт входных полей из конфигурации.
