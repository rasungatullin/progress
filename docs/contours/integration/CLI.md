...
Вместо `github_app_id` можно задать `github_app_client_id`; если заполнены оба поля, для `iss` JWT используется `github_app_client_id`. PEM-ключ можно передать через `github_app_private_key_private`; тогда значение читается из выбранного хранилища приватных значений и не записывается в проектную конфигурацию.
