...
Для GitHub App используется отдельная схема авторизации. Настройка интегрируемой системы содержит `github_app_id` или `github_app_client_id`, `github_app_installation_id`, а также `github_app_private_key_path` или `github_app_private_key_private`. Адаптер формирует JWT приложения, выпускает установочный токен через GitHub REST API и использует этот токен для последующих вызовов REST API и GraphQL API. JWT, установочный токен и содержимое PEM не входят в канонический запрос, структурированный вывод, журналы и диагностические строки.
...
Изменение классифицировано как `Целевой контракт`. Публичное дерево содержит только типо-ориентированные ветки `issue`, `repo`, `messenger`, `wiki` и служебные команды `operations`, `status`, `invoke`. Команды по именам внешних систем не входят в публичный контракт и скрыты из справки. Значения `tracker` и `repository` сохраняются только для чтения прежних настроек.
...
Внешние имена команд, например `github issue comments` или `bitbucket pr get`, не являются каноническими именами операций. Такие команды не входят в публичное дерево CLI; маршрутизация выполняется только через типо-ориентированные команды и `--system`.

Команды по именам внешних систем не являются переходным контрактом и не сохраняются как совместимые оболочки. Совместимость с прежними настройками, включая `github.json`, обеспечивается на границе чтения конфигурации и не расширяет публичное дерево CLI.
...
`Request.Extra` и `ProviderRequest.Extra` — карта дополнительных полей структурированного ввода сценарной операции. Сервис сохраняет в ней поля после проверки `OperationInputContract` и передаёт их сценарию без изменения имён и значений. Поле не предназначено для секретов, не заменяет типизированные поля встроенных операций и не используется для обхода проверки обязательных полей.
...
Для сценарных систем типа `script` каталог строится из `systems.<name>.operations`: обязательные и необязательные поля берутся из конфигурации операции, а значения по умолчанию попадают в описатель поля. Это позволяет контуру исполнения увидеть контракт сценарной операции до фактического запуска внешнего сценария. Если сценарная система включена, операции публикуются как доступные через зарегистрированный адаптер `script`.
...
Сценарный адаптер подключает внешний трекер или служебную систему без отдельного Go-адаптера. Для остальных контуров такая система остаётся обычной интегрируемой системой: вызов идёт через `Request`, ответ возвращается как `Response`, а операции называются каноническими именами.

Минимальная конфигурация операции:
...
Перед запуском адаптер проверяет обязательные поля, применяет значения по умолчанию, создаёт временный JSON-файл запроса и передаёт путь к нему в переменной `PROGRESS_INTEGRATION_REQUEST_FILE`. Сценарий запускается из корня репозитория или из директории `path`, если она указана в настройке системы.
...
Секреты из `token` и значение переменной из `token_env` не записываются во входной JSON. Если сценарию нужен секрет, он должен читать его из окружения процесса по согласованному имени.

Сценарий пишет в stdout JSON-ответ:
...
Для операций изменения сценарий может вернуть `operation_result`. Для отказа используется `failure` с каноническим `kind`. Невалидный JSON, неизвестная форма ответа и ошибки запуска переводятся в `internal-integration-error`, `invalid-request`, `timeout` или `external-failure`.
