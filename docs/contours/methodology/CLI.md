...
При сохранении навыков путь в поле `path` задаётся относительно соответствующего каталога `methodology/skills/`. Общий и проектный слой могут содержать записи с одинаковым именем: при просмотре будет показана проектная запись, а её `source` будет `local`.
