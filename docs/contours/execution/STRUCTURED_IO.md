...
8. если разрешённый профиль требует структурированный вывод, в директиву добавляется самодостаточная инструкция о структуре ожидаемого результата: обязательный непустой `summary`, формы объектных секций и компактный канонический JSON-пример только для выбранных действием опциональных полей `structured_output_fields`;
...
- `prompt-additions`;
- `prompt-additions-file`;
- `structured-output`;
- `structured-output-required`.

Действие задаёт список полей структурированного вывода `structured_output_fields`. Этот список передаётся операциям сборки исполнительной директивы и разбора результата.

Поля `prompt-additions` и `prompt-additions-file` задаются в `defaults` и/или конкретном профиле. `prompt-additions-file` содержит путь относительно корня репозитория и должен указывать на файл внутри него. Содержимое файла загружается как одно текстовое дополнение. Дополнения объединяются в следующем порядке: значения `prompt-additions` из `defaults`, файл из `defaults`, значения конкретного профиля, файл конкретного профиля. Этот текст остаётся обычной текстовой частью исполнительной директивы и не смешивается с каноническим структурированным вводом.

`prompt-additions-file` относится к `Целевому контракту`. Источником истины служит `.progress/execution/profiles.json`; поле применяется к секциям `defaults` и `profiles` при разрешении исполнительного профиля.

Пример вынесения большой инструкции профиля в отдельный Markdown-файл:

```json
{
  "profiles": {
    "review": {
      "prompt-additions-file": ".progress/methodology/instructions/review-pull-request-directive.md"
    }
  }
}
```

Если файл отсутствует, путь абсолютный или выходит за корень репозитория, разрешение профиля завершается диагностируемой ошибкой. Прежняя форма `prompt-additions` остаётся совместимой и может использоваться без файла.

Проектная конфигурация разделяет содержательные инструкции по действиям: профиль `coder` используется для первичной реализации, `review-rework` — для доработки по замечаниям ревизии, `review` — для ревизии запроса на слияние. Профили `coder` и `review-rework` используют один binding `coder`, но загружают разные файлы инструкций.

Поле `structured_output_fields` задаётся в действии и влияет на инструкцию в исполнительной директиве: оно определяет, какие дополнительные верхнеуровневые канонические поля исполнительный модуль должен по возможности заполнить. То же значение передаётся операции разбора результата. Значение `summary` допускается для совместимости с прежним контрактом, но остаётся обязательным и не делает поле настраиваемым. Канонический разбор при этом не сужается и по-прежнему принимает любой валидный JSON с дополнительными поддерживаемыми секциями.

Изменение относится к `Целевому контракту`. Источником истины служит описание действия в каталоге методик. Граница применения — сборка исполнительной директивы и разбор результата этого действия. Настройка профиля `structured-output-fields` удалена; проектные конфигурации должны перенести соответствующий список в `structured_output_fields` действия.
