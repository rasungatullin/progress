...
В текущей реализации этот канонический `structured output` выражается через единый JSON-контракт верхнего уровня с полями `protocol_version`, `summary`, `remarks`, `questions`, `follow_up_actions`, `changes`, `commands`, `conclusion`, `extensions`.
