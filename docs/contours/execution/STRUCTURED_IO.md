...
Ответ на замечание в `review_responses` может содержать поле `type`: значение `inline` требует `thread_id`, публикует ответ в цепочку и закрывает её при разрешённом состоянии; значение `comment` публикует новый связанный комментарий без закрытия цепочки; значение `local` сохраняет ответ только в результате и не вызывает внешний контур. Для `remark_id` следует возвращать устойчивое поле `id` из канонического замечания; внешний `external_id` сохраняется в том же замечании для интеграционного контура. Вид ответа и `thread_id` восстанавливаются только по каноническому замечанию с указанным `remark_id`; явно переданные значения не подменяют каноническую цель. Если соответствие отсутствует или неоднозначно либо вид невозможно восстановить, ответ отклоняется без внешнего побочного эффекта.
