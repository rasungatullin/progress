...
5. `structured-output-fields` наследуется из `defaults` или целиком переопределяется профилем и управляет только тем, какие канонические optional-поля executor просит у runner в prompt;
...
3. если в профиле задан `structured-output-fields`, executor просит у runner только перечисленные optional-поля, но parser по-прежнему принимает любой валидный канонический `review-cycle/v1` payload с дополнительными секциями.
