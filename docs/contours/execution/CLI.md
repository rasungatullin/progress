...
Runner может дополнительно вернуть необязательный structured block в формате JSON внутри секции `<progress-structured-output>...</progress-structured-output>`. Поддерживаются ключи `critical_remarks`, `minor_remarks` и `questions`, каждый со значением-массивом строк. При успешном разборе executor сохраняет обычный текст как `summary`, а structured block печатает отдельными диагностическими строками. Если structured block отсутствует, поведение остаётся прежним. Если block присутствует, но не парсится, executor сохраняет исходный runner output в `summary` без заполнения дополнительных полей, чтобы не терять диагностический контекст.
