...
- `--review-profile` — профиль ревизии.

`--max-executions` ограничивает количество запусков исполнения. Значение по умолчанию равно `5`.

`review-cycle` не вводит отдельный путь подготовки среды. Сначала запускается исполнение, затем ревизия. Если структурированный вывод ревизии содержит `conclusion.status=ok`, `approve` или `approved`, цикл завершается успешно. Любой другой статус считается требованием доработки, и следующий запуск исполнения получает результаты предыдущего исполнения и замечания ревизии через `StructuredInput.review_remarks` и `StructuredInput.previous_run_results`. Если лимит исполнений исчерпан без положительного заключения, итоговый статус команды становится `limit-reached`.
...
progress execution review-cycle \
  ...
  --task "Исполнить задачу и провести саморевизию результата"
```

### 3.3 `progress execution resume`

Команда возобновляет ранее сохранённый сеанс исполнительного модуля и передаёт в него только дополнительное структурированное сообщение, не копируя полный контекст исходной задачи из родительского запуска.

Поддерживаются флаги:

- `--run <id|latest>` — исходный запуск, от которого возобновляется сеанс;
- `--message <text>` — дополнительное сообщение;
- `--message-file <path>` — прочитать дополнительное сообщение из файла;
- `--name <name>` — фильтр имени запуска для выбора `latest`;
- `--profile <name>` — опциональное переопределение исполнительного профиля;
- `--structured-output` и `--structured-output-required` — сохранить текущие правила structured output;
- `--dry-run` — показать собранное задание на возобновление без запуска исполнительного модуля.

Флаги `--message` и `--message-file` взаимоисключающие. Пустое дополнительное сообщение считается ошибкой.

При возобновлении контур исполнения читает запись родительского запуска из контекстного архива, берёт из неё `runner`, `model`, `launch_directory` и внешний идентификатор сеанса исполнительного модуля, затем строит новый компактный `structured input`:

- исходная `task` не копируется;
- новое сообщение добавляется в `operational_context[]`;
- компактная ссылка на родительский запуск добавляется в `previous_run_results[]`;
- служебные метаданные возобновления сохраняются в `extensions.resume`.

Если исполнительный модуль поддерживает сохранённые сеансы, контур исполнения вызывает его нативный режим возобновления. В текущей реализации поддерживаются:
...
progress execution resume \
  --run latest \
  --name task-resume-session \
  --message "Учти новое ограничение и продолжи работу"
```

### 3.4 `progress execution dispatcher`
...
### 3.5 `progress execution profile`
...
```json
{
  ...
  "profiles": {
    ...
    "coder": {
      "description": "Профиль кодирования с обязательным структурированным выводом для каскадной обработки",
      "model-binding": "coder",
      "structured-output": true,
      "structured-output-fields": ["commit_message", "changes", "remarks", "commands"]
    },
    "review": {
      "description": "Профиль ревизии без автоматического коммита и отправки ветки",
      "model-binding": "review",
      "prompt-additions": [
        "Ты выполняешь ревизию кода. Не изменяй код и не создавай коммит.",
        "Сначала собери контекст запроса на слияние, задачи, diff и предыдущих замечаний ревизии, если они указаны во входе.",
        ...
        "Верни структурированный вывод с remarks, questions, follow_up_actions и conclusion."
      ],
      "structured-output": true,
...
Отдельный файл слоя может отсутствовать целиком, но если он присутствует — конфигурация обязана содержать полный формат (`runners`, `models`, `bindings`), т.е. «неполные» фрагменты не поддерживаются. Слои объединяются после отдельной валидации каждого: `runners` объединяются как карта конфигураций с переопределением одноимённых записей локальным слоем, список `models` объединяется без дублей, локальный `defaults.model-binding` переопределяет глобальный, а локальная `bindings` с тем же именем перезаписывает глобальную.
...
Итоговый файл хранит конфигурации доступных `runners`, список `models`, `bindings` и опциональный `defaults.model-binding`. Поле `runners` задаётся объектом, где ключ является именем исполнительного модуля, а значение описывает тип, включение, типизированные настройки встроенного адаптера или путь к внешнему сценарию. Поле `models` задаётся массивом строк, например `["openai/gpt-5.4", "gpt-5.3-codex"]`. Семантические привязки вроде `default`, `coder` и `review` указывают на конкретную пару `runner/model`.
...
```json
{
  ...
  "models": ["gpt-5.3-codex", "pi/default"],
  "bindings": {
    "coder": {"runner": "codex", "model": "gpt-5.3-codex"},
    "pi": {"runner": "pi", "model": "pi/default"}
  }
}
```

Поддерживаемые значения `runners.<name>.type`:

- `codex` — встроенный адаптер команды `codex exec`; произвольные `command`, `args` и `env` не поддерживаются, настройка `settings.sandbox` преобразуется адаптером в параметр песочницы;
- `opencode` — встроенный адаптер команды `opencode run`; произвольные `command`, `args` и `env` не поддерживаются;
- `script` — внешний сценарий запуска. Для него обязательно поле `script`; сценарий запускается из рабочего каталога задачи.

Для `script`-модуля контур исполнения собирает итоговую исполнительную директиву, сохраняет её в служебный файл `.progress/runner-input/` и передаёт путь через переменную окружения `PROGRESS_RUNNER_PROMPT_FILE`. Минимальный контракт окружения:

- `PROGRESS_RUNNER_NAME` — имя исполнительного модуля, например `pi`;
- `PROGRESS_RUNNER_MODEL` — выбранная модель, например `pi/default`;
- `PROGRESS_RUNNER_PROMPT_FILE` — путь к файлу с итоговой исполнительной директивой.

`PROGRESS_RUNNER_DIRECTORY` не является частью обязательного контракта: сценарий запускается непосредственно из рабочего каталога задачи. Возобновление сценарных модулей на текущем этапе не поддерживается.
...
4. `allow-model-fallback` наследуется из `defaults` и разрешает резервный выбор через `resources.defaults.model-binding`, если профиль ссылается на неизвестную привязку или не задаёт `model-binding`; если резервный выбор запрещён, привязку по умолчанию использовать нельзя;
...
6. `prompt-additions` наследуется из `defaults` и объединяется с дополнениями конкретного профиля; дополнения из `defaults` идут первыми, дополнения профиля идут после них;
7. `structured-output-fields` наследуется из `defaults` или целиком переопределяется профилем, поддерживает `summary` и остальные канонические top-level поля, но влияет только на те дополнительные секции, которые исполнительный модуль отдельно перечисляет в директиве;
...
В разрешённом профиле команда явно возвращает `description`, `mode`, `model-binding`, `allow-model-fallback`, `prompt-additions`, `structured-output`, `structured-output-required`, `structured-output-fields` и `commit-push`. Значение `commit-push` по умолчанию безопасное и равно `false`, но может использоваться последующей стадией запуска как унаследованный признак автоматического коммита и отправки ветки.

### 3.6 `progress execution resources`
...
### 3.7 `progress execution workplace`
...
Если `--repo` не передан, команда сохраняет прежнее поведение и подготавливает локальное рабочее место в `.progress/workplaces/<name>` текущего git-репозитория. Если каталог ещё не существует, команда создаёт `git worktree` с локальной веткой `<name>` от актуальной ветки по умолчанию удалённого `origin`. Если каталог уже существует, команда не пересоздаёт его, а проверяет, что это валидный `git worktree` и что в нём активна ветка `<name>`.

Если `--repo` передан, поддерживаются форматы `https://github.com/owner/name`, `https://github.com/owner/name.git`, `git@github.com:owner/name.git` и краткая форма `owner/name`. После нормализации clone URL локальная кэш-копия репозитория материализуется в `.progress/repositories/<repo-key>`, затем `worktree` создаётся в `.progress/workplaces/<repo-key>/<name>`. Диагностический вывод команды дополнительно печатает выбранный `repository=` и локальный `repository-root=`.

### 3.8 `progress execution launch`
...
Изолированный запуск без ресурсного снабжения поддерживает минимум два встроенных значения `runner`:
...
На полном маршруте `progress execution start` конкретные `runner/model` и конфигурация исполнительного модуля берутся не из профиля, а из разрешённого выделения ресурсов. Если пользователь передал `--model-binding`, он имеет приоритет над профилем. Если пользователь передал явные `--runner` и `--model`, подсистема ресурсов использует их без привязки, но только если оба значения зарегистрированы в `resources.json`, а выбранный исполнительный модуль не выключен через `enabled: false`. Если резервный выбор действительно нужен, но `resources.defaults.model-binding` не задан, подсистема ресурсов возвращает диагностируемую ошибку вместо молчаливого выбора модели.
...
Если ни `--structured-output`, ни `--structured-output-required`, ни разрешённый профиль не включают структурированный вывод, исполнительный модуль не добавляет в директиву никакой новой служебной structured-инструкции и поведение остаётся максимально близким к обычному текстовому запуску.

Если разрешённый профиль содержит `prompt-additions`, исполнительный модуль добавляет их в итоговую директиву после задачи из структурированного ввода и до любых structured-инструкций.

Если `--structured-output`, `--structured-output-required` или разрешённый профиль эффективно включают структурированный вывод, исполнительный модуль сам добавляет в итоговую директиву самодостаточную каноническую инструкцию, требующую завершающий блок `<progress-structured-output>...</progress-structured-output>` и канонический JSON-объект структурированного вывода. Инструкция всегда требует непустой `summary`, а формы объектных секций и канонический JSON-пример строятся с учётом выбранного набора: только для выбранных опциональных полей из `structured-output-fields` или для полного набора, если поле не задано. В текущей реализации итоговый порядок такой: структурированный ввод -> `prompt-additions` профиля -> инструкция структурированного вывода -> JSON-контекст структурированного ввода.

Профиль `review` служит пресетом для ревизии кода и сам добавляет минимальную служебную инструкцию про сбор контекста запроса на слияние, запрет на изменение кода, приоритет дефектов, регрессий и пропущенных проверок, проверку закрытия предыдущих замечаний и требование вернуть положительное заключение при отсутствии блокирующих замечаний. Поэтому короткого вызова вроде `progress execution start --profile review --task "Проведи ревизию PR #38 в rasungatullin/progress"` достаточно: дополнения профиля автоматически расширят задачу до полноценной ревизионной директивы.

Поддерживается один канонический структурированный вывод:

1. исполнительный модуль должен вернуть JSON-объект с непустым `summary`;
...
3. если в профиле задан `structured-output-fields`, исполнительный модуль запрашивает только перечисленные дополнительные поля; `summary` остаётся обязательным всегда, а parser по-прежнему принимает любой валидный канонический payload с дополнительными секциями.

Флаг `--structured-output-required` и одноимённое поле профиля объединяются по OR-семантике и включают строгую валидацию результата: ошибкой запуска считается отсутствие завершающего structured-блока, невалидный JSON-блок, пустой или бессмысленный payload, неизвестные top-level поля, пустые объектные элементы внутри секций структурированного вывода, а также отсутствие обязательного непустого `summary`. Для `json.UnmarshalTypeError` строгий parser возвращает более полезную диагностику в формате `type mismatch at <field>: expected <type> but got <json-kind>`. При этом свободный текст ответа не теряется: если завершающий блок невалиден, он остаётся в `summary`, а строгий режим возвращает диагностичную schema-level ошибку.
...
Канонический структурированный ввод является единственным внутренним представлением входного контекста. Поддерживается следующая схема верхнего уровня: `task`, `constraints[]`, `project_context[]`, `operational_context[]`, `previous_run_results[]`, `review_remarks[]`, `review_responses[]`, `integration_actions[]`, `extensions`.

Канонический структурированный вывод является единственным внутренним представлением структурированного результата. Поддерживается следующая схема верхнего уровня: `summary`, `commit_message`, `remarks[]`, `questions[]`, `follow_up_actions[]`, `changes[]`, `commands[]`, `conclusion`, `extensions`. Поле `commit_message` остаётся необязательным и используется только как кандидат для сообщения коммита при включённом `commit-push`. Для `remarks[]` доступны поля `id`, `status`, `severity`, `type`, `title`, `body`, `answer`, `resolution`. Для дальнейшего расширения верхнеуровневый контейнер `extensions` сохраняется отдельной ветвью, а повторяемые секции вынесены в самостоятельные типы.

Пример неправильной и канонической формы JSON:
...
- замечания ревизии могут быть автоматически прикреплены к коду через интеграции;
...
Структурированный ввод для полного маршрута задаётся явно через `--input-file` и structured-input флаги. Передача входного контракта через завершающий блок `<progress-structured-input>...</progress-structured-input>` внутри `--prompt` больше не поддерживается. Низкоуровневый `progress execution launch` сохраняет `--prompt` как ручной запуск исполнительного модуля, но полный маршрут `start` и `review-cycle` строят директиву внутри контура исполнения из структурированного ввода.

Целевой механизм должен быть расширяемым через конфигурацию. Это означает, что схема структурированного ввода и вывода, правила включения отдельных секций в директиву и набор допустимых команд или заключений не должны быть жёстко зашиты только в одном исполнительном пути. В текущей реализации исполнительный профиль уже управляет включением `structured output`, строгим режимом и списком опциональных полей структурированного вывода для инструкции, а структурированный ввод определяется `LaunchSpec.StructuredInput`, собранным из CLI-флагов или `--input-file`.

В CLI итог печатается в виде отдельного блока `summary<<PROGRESS_SUMMARY ... PROGRESS_SUMMARY`. Если сохранены локальные runtime-артефакты, CLI дополнительно печатает `raw-output-path=...` для полного stdout/stderr исполнительного модуля и `run-record-path=...` для JSON-записи запуска в `.progress/execution-runs/`. Запись запуска содержит нормализованный `Invocation`, разрешённый профиль, выделение ресурсов, рабочее место, канонический структурированный ввод, внешний идентификатор сеанса исполнительного модуля при наличии, сырой payload структурированного вывода, разобранный структурированный вывод и итоговый статус запуска. Для запусков, созданных через `execution resume`, контекстный архив дополнительно хранит `parent_run_id`, краткое дополнительное сообщение и его источник. Эти файлы считаются локальной диагностикой и не попадают в автоматический `commit-push`.

После этого при наличии структурированных данных выводится секция `structured-output:`. Для канонического структурированного вывода CLI дополнительно печатает строки `summary-field=...`, `commit-message=...` и JSON-строки `remark=...`, `question=...`, `follow-up-action=...`, `change=...`, `command=...`, `conclusion=...`, `extension=...`. Эти JSON-строки выводятся без дополнительной whitespace-нормализации, чтобы payload оставался без потерь.
...
4. нейтральный резервный вариант `Apply task result`, только если все предыдущие источники пустые или состоят из whitespace.

Пустой или состоящий только из пробелов `commit_message` не считается валидным и не блокирует переход к следующему источнику. Если `commit-push` не включён, структурированный вывод с `commit_message` по-прежнему парсится и печатается в CLI, но git-стадия не запускается.
...
Для `progress execution review-cycle` внешний маршрут становится циклическим, но каждый отдельный запуск исполнения и ревизии по-прежнему проходит тот же внутренний маршрут полного запуска.
...
Пример вызова только исполнительного модуля:

```bash
progress execution launch --dir .progress/workplaces/feature-brief --prompt "Подготовь изменения"
```

Пример вызова исполнительного модуля с последующим коммитом и отправкой ветки:
...
1. без `--commit-push` выполняется только исполнительный модуль;
2. с `--commit-push` или с `commit-push=true` у профиля git-операции запускаются только после успешного завершения исполнительного модуля;
3. при ошибке исполнительного модуля git-операции не выполняются;
