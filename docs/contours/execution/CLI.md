...
Команда должна уметь принять структурированную задачу, разрешить действие, подобрать профиль, определить модель и движок, подготовить рабочее место, выполнить задачу и вернуть структурированный результат. Конкретный прикладной сценарий, например кодирование, ревизия, доработка замечаний или синтез артефакта, должен определяться действием и структурированным входом, а не отдельным жёстко зашитым режимом команды.

В текущей реализации конкретный прикладной сценарий задаётся флагом `--action`. Действие определяет список операций текущего запуска. Профиль остаётся технической настройкой исполнения: он задаёт режим, привязку ресурсов, дополнительные части директивы и требования к структурированному выводу, но не выбирает класс действия.
...
Команда вызывает диспетчер исполнения как отдельный модуль. Данный режим предназначен для диагностики разрешённого действия, наблюдения за списком операций и последующей отладки правил диспетчеризации.

Команда принимает `--action` и `--profile`, разрешает действие тем же каталогом действий, что и полный запуск, и печатает операции в порядке их выполнения.
...
```json
{
  ...
  "profiles": {
    ...
    "review": {
      "description": "Профиль ревизии без изменения файлов",
...
В разрешённом профиле команда явно возвращает `description`, `mode`, `model-binding`, `allow-model-fallback`, `prompt-additions`, `structured-output`, `structured-output-required` и `structured-output-fields`. Завершающие git-операции не являются свойством профиля и должны задаваться операциями действия.
...
- наличие операции `commit-push` в действии или явного флага прямого запуска;
- причины перехода к каждой стадии диспетчера.

Если выполняется операция `commit-push` или прямой запуск `progress execution launch --commit-push`, то после успешного завершения исполнительного модуля команда:
...
Пустой или состоящий только из пробелов `commit_message` не считается валидным и не блокирует переход к следующему источнику. Если операция `commit-push` или явный флаг прямого запуска не включены, структурированный вывод с `commit_message` по-прежнему парсится и печатается в CLI, но git-стадия не запускается.
...
1. разрешение действия;
2. выполнение операций действия;
3. выбор профиля внутри операции `resolve-profile`, если действие требует синтеза;
4. проверка и резервирование ресурсов внутри операции `allocate-resources`;
5. подготовка рабочего места внутри операции `prepare-workplace`;
6. пуск задачи внутри операции `launch-synthesis`;
7. разбор и фиксация результата.
...
2. с `--commit-push` git-операции запускаются только после успешного завершения исполнительного модуля;
