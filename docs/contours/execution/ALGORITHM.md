...
Если во входе задан источник репозитория, модуль сначала материализует локальную кэш-копию этого репозитория в служебном каталоге текущего проекта, проверяет `origin`, выполняет `git fetch`, а затем создаёт `worktree` от ветки по умолчанию целевого удалённого `origin`. Если источник репозитория не задан, используется текущий git-репозиторий процесса и прежний локальный маршрут подготовки рабочего места.
...
Для управляемого режима саморевизии используется команда `progress execution review-cycle`. Внешний координатор этой команды запускает обычный маршрут исполнения, затем обычный маршрут ревизии, проверяет `structured_output.conclusion.status` результата ревизии и при отсутствии положительного статуса повторяет исполнение. Повторный запуск получает результаты предыдущих шагов и замечания ревизии через канонический `StructuredInput`. Сам маршрут `profile -> resources -> workplace -> launch` при этом не дублируется и не заменяется отдельной веткой.
