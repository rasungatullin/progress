...
Схема отражает минимальный порядок для действия инженерного синтеза. В текущей реализации этот порядок не должен быть зашит в сервис исполнения напрямую: контур сначала разрешает имя действия через каталог методик, затем проходит список операций, закреплённый в `Action.Operations` сохранённого действия.
...
Если действие не требует содержательного синтеза, операции синтеза отсутствуют в его составе. Контур исполнения не запускает лишние операции с последующим переводом в состояние `skipped`.
