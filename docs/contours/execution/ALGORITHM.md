...
- требования к structured output и prompt additions;
- требования к рабочему месту;
- условия завершения и `commit-push`.

### 4.3 Подсистема ресурсного снабжения

Подсистема ресурсного снабжения проверяет наличие требуемых ресурсов и выполняет предварительное резервирование ограниченных единиц. На этой стадии `model-binding` или явная пара `runner/model` разрешаются в фактическую аллокацию запуска. Конфиг ресурсов хранит `runners` и `models` как массивы строк; fallback на `defaults.model-binding` допустим только при явно разрешённой policy, а отсутствие default binding в таком сценарии считается диагностируемой ошибкой.

К контролируемым сущностям относятся:

- зарегистрированные `runners`;
- зарегистрированные `models`;
- семантические `bindings runner+model`;
...
Если во входе задан источник репозитория, модуль сначала materialize-ит локальную cache-копию этого репозитория в служебном каталоге текущего проекта, проверяет `origin`, выполняет `git fetch`, а затем создаёт worktree уже от default branch целевого remote `origin`. Если источник репозитория не задан, используется текущий git-репозиторий процесса и прежний локальный flow подготовки рабочего места.
...
Данный модуль не выполняет повторного согласования профиля и не резервирует ресурсы заново. Его назначение состоит в том, чтобы начать выполнение задачи в уже подготовленной среде с уже разрешённой аллокацией `runner/model`.
...
5. из структурированного ввода и текстовой постановки собирается итоговая директива;
6. выполняется пуск задачи;
7. разбирается структурированный вывод и фиксируется итог исполнения;
...
В одиночном режиме отказ на любой стадии завершает текущий цикл исполнения без автоматической повторной попытки.

Для управляемого режима саморевью используется команда `progress execution review-cycle`. Внешний координатор этой команды запускает обычный маршрут исполнения, затем обычный маршрут ревью, проверяет `structured_output.conclusion.status` результата ревью и при отсутствии approving status повторяет исполнение. Повторный запуск получает результаты предыдущих шагов и замечания ревью через канонический `StructuredInput`. Сам маршрут `profile -> resources -> workplace -> launch` при этом не дублируется и не заменяется отдельной веткой.
