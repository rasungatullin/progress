...
К настройкам интеграции относится и выбор хранилища приватных значений, из которого адаптеры получают токены и ключи авторизации по ссылке из конфигурации. Управление хранилищем выполняется через `progress configuration private`; старый путь `progress integration private` сохранён для совместимости.
